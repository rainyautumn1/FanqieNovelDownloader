...
## [2025-12-29] - 字符映射修复与问题排查

### 🐛 问题修复
- **核心字符映射修正**:
  - 修复了严重字符映射错误：字符 "直" 在下载时被错误映射为 "首"（例如 "直到" 变为 "首到"）。
  - 更新了内部混淆字符集映射表 (`charset`)，修正了 Unicode 偏移量对应的字符。
- **调试工具**:
  - 新增字符映射调试脚本，用于快速定位和验证混淆字符问题。

## [2025-12-26] - EPUB阅读体验升级与更新源调整

### ✨ 新增功能与优化
- **EPUB 电子书体验大升级**:
  - **专业排版引擎**: 引入全新 CSS 样式系统，优化字体渲染（优先调用系统宋体）、行间距与段落缩进，完美复刻纸质书阅读质感。
  - **精美书籍扉页**: 告别简陋文本，新增卡片式书籍信息页，优雅展示封面、作者与简介。
  - **沉浸式目录**: 重构目录页面 (`toc.xhtml`)，去除冗余编号，增加虚线引导与交互动效，与正文风格浑然一体。
  - **兼容性增强**: 修复特殊字符（如 `<` `>` `&`）导致的 XML 解析错误，彻底解决部分阅读器显示 "xml version" 代码的问题。
- **图文资源全覆盖**:
  - **插图引擎重写**: 全面支持章节内插图的解析与下载，自动识别多种图片格式 (.jpg/.png/.webp/.gif)，并在 EPUB 中完美展示。
  - **封面获取增强**: 修复了批量下载模式下封面可能丢失的问题，并增强了封面图片的格式兼容性。
- **基础设施升级**:
  - **极速更新源**: 软件更新与公告服务全线迁移至**腾讯云 COS**，确保国内用户秒级响应，告别连接超时。
- **安全与隐私**:
  - **高级指纹混淆**: 引入动态 User-Agent 池与 Headers 伪装技术。每次启动随机模拟不同浏览器（Chrome/Edge/Firefox/Safari）及对应平台特征，并确保内置浏览器与下载引擎指纹严格同步，有效降低风控触发概率。
